...
app.post('/api/tasks', (req, res) => {
    ...
    // SQL to insert new task. Default status to 'TODO'
    const sql = `
    ...
    db.run(
        sql, 
        [title, description || null, priority || 'MEDIUM'], // description and priority are optional
        function(err) {
            if (err) {
...
app.patch('/api/tasks/:id/status', (req, res) => {
    ...
    db.run(sql, [status, id], function(err) {
        ...
        res.json({ message: `Task status updated to ${status}`, changes: this.changes });
    });
});
